...
API_ID = int(environ.get('API_ID', '14840137')) # API ID from my.telegram.org
API_HASH = environ.get('API_HASH', 'ca365d2391f77f2d2d083b879789639c')  # API Hash from my.telegram.org
BOT_TOKEN = environ.get('BOT_TOKEN', "7786950860:AAFQTBjwchD0Qhlj-idDYknH5-yZcseMAMY")    # Bot token from @BotFather
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '5692738726').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1003449214123').split()]  # Channel id for auto indexing (make sure bot is admin)

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1003322562324'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1003322562324'))  # Bin channel id (make sure bot is admin)
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1003322562324'))  # Premium logs channel id
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1003317084724').split()] #(make sure bot is admin)
...
# FORCE_SUB 
auth_req_channels = environ.get("AUTH_REQ_CHANNELS", "-1002580207283 -1002712787457")# requst to join Channel for force sub (make sure bot is admin) only for bot ADMINS  
...
# ============================
DATABASE_URI = environ.get('DATABASE_URI' "mongodb+srv://jithuabi619_db_user:5Zs9yxvGQM9Jw4qY@cluster0.tpnfaz3.mongodb.net/?appName=Cluster0")  # MongoDB URI for the database
...
MOVIE_UPDATE_CHANNEL = int(environ.get('MOVIE_UPDATE_CHANNEL', '-1003310920154'))  # Notification of sent to your channel
...
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/MovieTime') # Group link for the bot
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/MovieTime') # Owner link for the bot
...
PORT = int(environ.get("PORT", "18012"))  # Port for the web server (default: 8080)
